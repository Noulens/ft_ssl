...
void    *do_md5(void *data)
{
	...
	if ((to_digest->flags & e_one_op) || (to_digest->flags & e_p))
	{
		char *str = md5_readinput(to_digest, &ctx, STDIN_FILENO);
		print_input_digest(to_digest->flags, ctx.digest, ctx.buffer, MD5_DIGEST_LGTH, str);
		free(str);
